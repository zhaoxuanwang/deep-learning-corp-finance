...
This section documents findings from the two-step SMM estimation run on the risky debt model (`docs/06_risky_debt_smm_workflow.ipynb`). The SMM pipeline itself is validated on the frictionless basic-investment model (`docs/05_smm_validation.ipynb`), where bias < 0.003 and RMSE < 0.017 across all parameters. The model solver is benchmarked in `docs/03b_risky_debt_interp_benchmark.ipynb` and converges to machine-precision zero-profit residuals ($\sim 10^{-14}$). The oracle test at the true parameters passes ($Q = 1.23 \times 10^{-5}$), confirming that the solver-simulation-moment pipeline produces correct moments at the true $\beta$. Therefore, the issues below are properties of the estimation problem, not implementation bugs.

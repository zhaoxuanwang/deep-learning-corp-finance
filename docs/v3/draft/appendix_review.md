# review.md — Code and Pipeline Audit Checklist (Scientific Estimation Stack)
...
## ENV-1 — TF / TFP / Keras version incompatibility (the Keras 3 break) `[BLOCKER]`
...
## ENV-2 — Mixed single and double precision (float32 vs float64) `[HIGH]`
...
## REPRO-1 — Nondeterminism and false reproducibility claims (TensorFlow) `[HIGH]`
...
## REPRO-2 — Random-number generators not coordinated across NumPy, SciPy, and TF `[HIGH]`
...
## REPRO-3 — Simulated data not reproducible during training or estimation `[HIGH]`
...
## TF-1 — A sped-up function is rebuilt on every call (retracing) `[HIGH]`
...
## TF-2 — Ordinary Python statements inside a sped-up function `[MED]`
...
## TF-3 — Gradients silently come back missing `[BLOCKER]`
...
## NUM-1 — Cholesky silently returns `NaN` on a non-positive-definite matrix `[BLOCKER]`
...
## NUM-2 — Products of probabilities underflow unless computed in log space `[HIGH]`
...
## OPT-1 — A SciPy or NumPy optimizer wrapping a TensorFlow objective `[HIGH]`
...
## OPT-2 — Derivative-free optimizers can report success at a non-optimum `[HIGH]`
...
## OPT-3 — Value function iteration does not converge or converges to the wrong fixed point `[HIGH]`
...
## OPT-4 — Optimizer choice and saved optimizer state `[MED]`
...
## EST-1 — SMM without common random numbers `[BLOCKER]`
...
## EST-2 — Method-of-moments Jacobian and weighting matrix `[MED]`
...
## EST-3 — NUTS is slow, especially through an expensive log-density `[HIGH]`
...
## EST-4 — NUTS step-size adaptation wired incorrectly `[HIGH]`
...
## EST-5 — Sampler output not checked for correctness or convergence `[BLOCKER]`
...
## NN-1 — Information leaks from the test set into training `[BLOCKER]`
...
## NN-2 — Saving and reloading a trained model changes its behavior `[HIGH]`
...
## HW-1 — GPU or Apple Metal results differ from the CPU `[HIGH]`
...
## DATA-1 — Loading a real dataset introduces silent corruption `[HIGH]`

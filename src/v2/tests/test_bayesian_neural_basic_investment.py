...
def test_log_likelihood_differentiable_in_beta(param_env, untrained_nn):
    ...
    beta_t = {k: tf.constant(float(v), tf.float32) for k, v in beta_true.items()}
...
def test_generate_frictionless_panel_shapes_and_positivity(param_env):
    ...
    assert set(panel.keys()) == {"k", "z"}
    assert panel["k"].shape == (64, 20)
    assert panel["z"].shape == (64, 20)
    assert np.all(panel["k"] > 0)
    assert np.all(panel["z"] > 0)
...
def test_generate_frictionless_panel_matches_analytical_relation(param_env):
    """Within the panel, ``log k_{t+1} - κ`` should equal
    ``(ρ/(1-α)) · log z_t`` exactly (no AR(1) noise in the relation —
    that noise enters z, not the policy)."""
    beta = neural_true_beta(param_env)
    panel = generate_frictionless_panel(
    ...
    log_k = np.log(panel["k"])    # log k_{t+1} at original t
    log_z = np.log(panel["z"])    # log z_t

    # The closed-form policy: log k_{t+1} = slope · log z_t + κ. Since panel
    # k[:, t] is k_{t+1} given z_t at the same index, the relation holds
    # column-wise.
    residual = log_k - (slope * log_z + kappa)
...
def test_derive_env_bounds_from_panel_basic(param_env):
    ...
    # Use the default log_z_half_width (1.0); verify against that.
    bounds = derive_env_bounds_from_panel(
        panel["k"], k_margin=1.2, mu=0.0,
    )
    assert set(bounds.keys()) == {"k_min", "k_max", "z_min", "z_max"}
    # k bounds include the panel with margin.
    assert bounds["k_min"] < float(panel["k"].min())
    assert bounds["k_max"] > float(panel["k"].max())
    ...
    # z range is sane (≈ e ≈ 2.72): NN training distribution lives where the
    # closed-form k'(z; β) is unclipped at the prior center.
...
def test_derive_env_bounds_z_independent_of_panel(param_env):
    ...
    b1 = derive_env_bounds_from_panel(p1["k"], log_z_half_width=3.0)
    b2 = derive_env_bounds_from_panel(p2["k"], log_z_half_width=3.0)
...
def test_derive_env_bounds_rejects_invalid_inputs(param_env):
    ...
    with pytest.raises(ValueError, match="k_margin"):
        derive_env_bounds_from_panel(panel["k"], k_margin=1.0)
    with pytest.raises(ValueError, match="log_z_half_width"):
        derive_env_bounds_from_panel(panel["k"], log_z_half_width=0.0)
...
def test_ekf_with_closed_form_mock_is_unbiased(param_env):
    ...
    beta_true = neural_true_beta(param_env, sigma_eta=0.05, sigma_xi=0.02)

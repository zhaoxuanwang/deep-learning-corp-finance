"""Tests for benchmark notebook helpers."""
...
class TestBenchmarkRunStorage:

    def test_prepare_benchmark_run_creates_dirs_and_latest(self, tmp_path):
        ctx = prepare_benchmark_run(
            "demo_benchmark",
            save_run=True,
        ...
        assert ctx["datasets_dir"].exists()
        latest = Path(tmp_path) / "demo_benchmark" / "latest"
        assert latest.is_symlink()
        assert latest.resolve() == ctx["run_dir"]

    def test_named_dataset_round_trip(self, tmp_path):
        ctx = prepare_benchmark_run(
            "demo_benchmark", save_run=True, results_root=str(tmp_path))
        flat = {
            "s_endo": tf.constant([[1.0], [2.0]], dtype=tf.float32),
            "z": tf.constant([[0.1], [0.2]], dtype=tf.float32),
            "z_next_main": tf.constant([[0.2], [0.3]], dtype=tf.float32),
            "z_next_fork": tf.constant([[0.25], [0.35]], dtype=tf.float32),
        }
        save_named_dataset(ctx, "val_flat", flat, fmt="flat")
        loaded = load_named_dataset(ctx, "val_flat", fmt="flat")
        for key in flat:
            np.testing.assert_allclose(loaded[key].numpy(), flat[key].numpy())
    ...
    def test_pfi_bundle_round_trip(self, tmp_path):
        ctx = prepare_benchmark_run(
            "demo_benchmark", save_run=True, results_root=str(tmp_path))
        result = {
        ...
        save_pfi_bundle(ctx, result, summary={"wall_time_sec": 12.5})
        loaded = load_pfi_bundle(ctx)
    ...
    def test_named_pfi_bundle_round_trip(self, tmp_path):
        ctx = prepare_benchmark_run(
            "demo_benchmark", save_run=True, results_root=str(tmp_path))
        result = {
        ...
        save_pfi_bundle(ctx, result, summary={"tau": 1e-3}, name="soft_tau_1e-3")
        loaded = load_pfi_bundle(ctx, name="soft_tau_1e-3")
        np.testing.assert_allclose(
    ...
    def test_method_bundle_round_trip(self, tmp_path):
        ...
        assert loaded["result"]["history"]["step"] == [0, 5]
        ...
        assert len(loaded["weight_history"]) == 2
        np.testing.assert_allclose(loaded["weight_history"][1][1][0], [[3.0], [4.0]])

    def test_manifest_sections_merge(self, tmp_path):
        ctx = prepare_benchmark_run(
            "demo_benchmark", save_run=True, results_root=str(tmp_path))
        save_manifest_sections(ctx, setup={"profile": "BALANCED"})
        save_manifest_sections(ctx, pfi={"grid": [200, 200, 2000]})
        with open(ctx["manifest_path"]) as f:
            manifest = json.load(f)
        assert manifest["setup"]["profile"] == "BALANCED"
        assert manifest["pfi"]["grid"] == [200, 200, 2000]
...
class TestBenchmarkPolicies:
    ...
    def test_restore_selected_snapshot(self):
        ...
        np.testing.assert_allclose(weights[0], [[3.0], [4.0]])
        np.testing.assert_allclose(weights[1], [0.2])

    def test_restore_selected_models_from_generic_checkpoint(self):

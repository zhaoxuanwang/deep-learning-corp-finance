...
# NOTE: euler_chi and euler_m are now in src/economy/logic.py
# The Euler primitives have been moved to the economy module for clean separation.
...
def compute_br_critic_diagnostics(
    ...
    Returns:
        Dict with:
            - cross_product: mean(delta1 * delta2) - the actual loss
            - mse_proxy: mean(0.5 * (delta1^2 + delta2^2)) - always positive
            - mae_proxy: mean(0.5 * (|delta1| + |delta2|)) - always positive
    """
    delta1 = V_curr - y1
    delta2 = V_curr - y2
